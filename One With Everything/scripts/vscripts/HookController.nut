...
local function PlayerRestricted(player){
	if(player != null){
		player.SetDisabled(true)
	}
}

local function PlayerReleased(player){
	if(player != null){
		...
		if(players.len() == 0){
			...
			for(local i=0; i<players.len();i++){
				if(players[i].GetEntity() != null && players[i].GetEntity().IsValid()){
					...
					if(NetProps.GetPropEntity(players[i].GetEntity(), "m_pummelAttacker") != null){
						PlayerRestricted(players[i])
					}
					if(NetProps.GetPropEntity(players[i].GetEntity(), "m_carryAttacker") != null){
						PlayerRestricted(players[i])
					}
					if(NetProps.GetPropEntity(players[i].GetEntity(), "m_pounceAttacker") != null){
						PlayerRestricted(players[i])
					}
					if(NetProps.GetPropEntity(players[i].GetEntity(), "m_jockeyAttacker") != null){
						PlayerRestricted(players[i])
					}
					
					
					if(NetProps.GetPropEntity(players[i].GetEntity(), "m_pummelAttacker") == null){
						PlayerReleased(players[i])
					}
					if(NetProps.GetPropEntity(players[i].GetEntity(), "m_carryAttacker") == null){
						PlayerReleased(players[i])
					}
