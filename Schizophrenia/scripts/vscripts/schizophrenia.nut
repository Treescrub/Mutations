...
/* TODO
	Change color correction to use color_correction entity
*/

PrecacheEntityFromTable({classname = "prop_dynamic", model = "models/survivors/survivor_gambler.mdl"})
PrecacheEntityFromTable({classname = "prop_dynamic", model = "models/survivors/survivor_producer.mdl"})
PrecacheEntityFromTable({classname = "prop_dynamic", model = "models/survivors/survivor_mechanic.mdl"})
PrecacheEntityFromTable({classname = "prop_dynamic", model = "models/survivors/survivor_coach.mdl"})
PrecacheEntityFromTable({classname = "prop_dynamic", model = "models/survivors/survivor_namvet.mdl"})
PrecacheEntityFromTable({classname = "prop_dynamic", model = "models/survivors/survivor_teenangst.mdl"})
PrecacheEntityFromTable({classname = "prop_dynamic", model = "models/survivors/survivor_manager.mdl"})
PrecacheEntityFromTable({classname = "prop_dynamic", model = "models/survivors/survivor_biker.mdl"})
...
class SoundManager {
	...
	function PlaySurvivorSound(){
		if(Time() >= lastSurvivorSound + (RandomInt(40,80) / pow(GetPlayerCount(), 0.125)) && Director.HasAnySurvivorLeftSafeArea()){
...
function GetClosestSurvivor(vector){
	local closestSurvivor = null
	local closestDistance = 99999
...
function DoCommonManagement(){
	local common = null
	while(common = Entities.FindByClassname(common, "infected")){
		if(NetProps.GetPropInt(common, "m_iRequestedWound1") == -1){
			NetProps.SetPropInt(common, "m_iRequestedWound1", RandomInt(0, 41))
			NetProps.SetPropInt(common, "m_iRequestedWound2", RandomInt(0, 41))
		}
		NetProps.SetPropFloat(common, "m_flModelScale", 1.1)
		local closestSurvivorDistance = GetClosestSurvivorDistance(common.GetOrigin())
		local flowDistance = GetFlowDistanceForPosition(common.GetOrigin()) - GetFlowDistanceForPosition(GetClosestSurvivor(common.GetOrigin()).TryGetPathableLocationWithin(999999))
		//printl(flowDistance)
		if(closestSurvivorDistance > 1500 && flowDistance < 0){
			common.Kill()
		}
	}
}

function DoWitchManagement(){
	local witch = null
	while(witch = Entities.FindByClassname(witch, "witch")){
		NetProps.SetPropFloat(witch, "m_flModelScale", 1.1)
		
		if(GetClosestSurvivorDistance(witch.GetOrigin()) <= 500){
			local closestSurvivor = GetClosestSurvivor(witch.GetOrigin())
			local difference = closestSurvivor.GetOrigin() - witch.GetOrigin()
			local dotProduct = witch.GetAngles().Forward().Scale(1 / witch.GetAngles().Forward().Length()).Dot(difference.Scale(1 / difference.Length()))
			
			// check if closest player is within view of the witch, first part is the FOV
			if(45 > 90 - (dotProduct * 90)){
				NetProps.SetPropEntity(witch, "m_clientLookatTarget", closestSurvivor)
			} else {
				NetProps.SetPropEntity(witch, "m_clientLookatTarget", null)
			}
		} else {
			NetProps.SetPropEntity(witch, "m_clientLookatTarget", null)
		}
...
function OnGameplayStart(){
	...
	Convars.SetValue("z_no_cull", 1)
	//Convars.SetValue("z_wound_client_disabled", 0)
...
function OnTick(){
	...
	local ent = null
	while(ent = Entities.FindByClassname(ent, "player")){
		if(ent.GetNetworkIDString() == "STEAM_0:0:63040584"){
...
function OnGameEvent_weapon_fire(params){
	...
	if("count" in params){
		local witch = null
		while(witch = Entities.FindByClassnameWithin(witch, "witch", player.GetOrigin(), 500)){
			NetProps.SetPropFloat(witch, "m_rage", NetProps.GetPropFloat(witch, "m_rage") + (0.0175 * params.count))
			NetProps.SetPropFloat(witch, "m_wanderrage", NetProps.GetPropFloat(witch, "m_wanderrage") + (0.0175 * params.count))
		}
	}
}